module Taggie
  class String < String
    ...
    def children
      @children ||= Array.new(inner_html.siblings_and_self(false).map! { |child| child.parent = self; child }, self)
    end
    ...
    def siblings_and_self(lookup_parent = true)
      lookup_parent && parent ? parent.children : Array.new(scan(inner_html_regex[0]).map! { |match| match[0].parent = parent; match[0] }, parent)
    end
